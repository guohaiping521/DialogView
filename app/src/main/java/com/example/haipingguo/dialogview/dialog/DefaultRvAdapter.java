...
class DefaultRvAdapter extends RecyclerView.Adapter<DefaultRvAdapter.DefaultVH> {
    ...
    DefaultRvAdapter(MDialog dialog, @LayoutRes int layout) {
        this.dialog = dialog;
        this.layout = layout;
    ...
    public DefaultVH onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        MUtils.setBackgroundCompat(view, dialog.getListSelector());
    ...
    public void onBindViewHolder(DefaultVH holder, int index) {
        ...
        boolean disabled = MUtils.isIn(index, dialog.builder.disabledIndices);
        int itemTextColor =
            disabled
                ? MUtils.adjustAlpha(dialog.builder.itemColor, 0.4f)
        ...
        switch (dialog.listType) {
            case SINGLE: {
                ...
                if (dialog.builder.choiceWidgetColor != null) {
                    MTintHelper.setTint(radio, dialog.builder.choiceWidgetColor);
                } else {
                    MTintHelper.setTint(radio, dialog.builder.widgetColor);
                }
            ...
            case MULTI: {
                ...
                if (dialog.builder.choiceWidgetColor != null) {
                    MTintHelper.setTint(checkbox, dialog.builder.choiceWidgetColor);
                } else {
                    MTintHelper.setTint(checkbox, dialog.builder.widgetColor);
                }
    ...
    interface InternalListCallback {
        ...
        boolean onItemSelected(
                MDialog dialog, View itemView, int position, CharSequence text, boolean longPress);
    }
